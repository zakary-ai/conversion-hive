...
const Email = ({ name, scheduledLabel, scheduledAt, meetingUrl, durationMinutes, closerName }: Props) => {
  ...
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Your discovery call is confirmed — Zoom link inside</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={card}>
            <Heading style={heading}>You're booked{name ? `, ${name}` : ''} 🎉</Heading>
            <Text style={muted}>
              Thanks for applying. Your discovery call{closerName ? ` with ${closerName}` : ''} is confirmed.
            </Text>
...
export const template = {
  component: Email,
  subject: 'Your discovery call is confirmed — Zoom link inside',
